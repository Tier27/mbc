...
jQuery(function($){
	...
	$start.change(function(){
		if ( !lanes_last_changed ) {
			$two_lanes_option.prop('disabled', true);
		...
		// If two lanes are available
		if( lane_availability > 1 ){
			if( $hours.val() == 1 ){
				// Allow the option for two lanes
				$two_lanes_option.prop('disabled', false);
				// Clear these to avoid duplication
				$bowlers.find('.two-lanes').remove();
				// Allow up to twelve bowlers
                $bowlers.append($bowlers_seven_to_twelve.clone());
			// If two hours are selected and there are two lanes available in the next hour
	...
	/*
	$('#book-reservation-submit').click(function(e){
		return;
		e.preventDefault();
		lanes = $lanes.val();
		bowlers = $bowlers.val();
		if( lanes == 1 && bowlers > 6 ) {
			alert("You can only have up to six bowlers on one lane");
			return false;
		}
	});
	*/
